...
label = 'Final_Heterodyned_Uniform'
...
class HeterodynedLikelihood():
    ...
    def maximize_likelihood(
        ...
        parameter_names = [param.name for param in parameters] + ["M_c", "eta"]
        def y(x):
        ...
        optimizer = optimization_Adam(
            n_steps=n_steps, learning_rate=0.0001, noise_level=1
        )
        ...
        while not jax.tree.reduce(
            ...
            guess = jnp.hstack([guess, M_c[:, None], eta[:, None]])
