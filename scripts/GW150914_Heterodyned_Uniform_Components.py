...
label = 'Final_Heterodyned_Uniform_Final'
...
# Define the priors.
parameters = [
    ParameterPrior("M_1", r"$M_c$", UniformPrior, 10.0, 80.0),
    ParameterPrior("M_2", r"$q$", UniformPrior, 10.0, 80.0), 
...
M_1_index = parameter_names.index("M_1")
M_2_index = parameter_names.index("M_2")

# | Define the log prior function
@jax.jit
def logprior_fn(x):
    M_1, M_2, *rest = x.T
    logprob_mass = jnp.where(M_1 < M_2, -jnp.inf, parameters[0].logprob(M_1) + parameters[1].logprob(M_2) + jnp.log(2))
    return jnp.sum(jnp.array([logprob_mass] + [param.logprob(val) for param, val in zip(parameters[2:], rest)]))
...
M_1, M_2 = initial_particles[:, M_1_index], initial_particles[:, M_2_index]
new_M_1 = jax.lax.select(M_1 < M_2, M_2, M_1)
new_M_2 = jax.lax.select(M_1 < M_2, M_1, M_2)
initial_particles = initial_particles.at[:, M_1_index].set(new_M_1)
initial_particles = initial_particles.at[:, M_2_index].set(new_M_2)
